...
function placeShips() {
    ...
    function ask(i) {
        // Condizione di uscita: tutte le navi sono state posizionate
        if (i === 6) {
        ...
        rl.question(`Nave ${i + 1}: `, input => {
            ...
            // Aggiornamento stato locale e salvataggio coordinate
            myBoard[y][x] = "S";
            ships.push({
                name: "Ship",
                size: 1,
            ...
            //Inserimento della nave successiva
            ask(i + 1);
        });
    }
...
function attack() {
    rl.question("Attacca (x y): ", input => {
        const [x, y] = input.split(" ").map(Number);
        // Controllo base validità coordinate prima dell'invio al server
        if (x >= 0 && x <= 9 && y >= 0 && y <= 9) {
